<h1 align="center">
  ¡Hola! 👋
  <br>
  Soy Miguel Sanguña 😼
</h1>
<img src="https://capsule-render.vercel.app/api?type=waving&color=D8B4FE&height=100&section=header" width="100%" />

## 🚀 Sobre mí
- 🎓 Estudiante de Desarrollo de Aplicaciones Multiplataforma (DAM), con una media de 8.71/10 en el primer curso
- ⚡ Me mueve construir cosas que funcionen de verdad, no solo que se vean bien
- 💻 Conocimiento en desarrollo de aplicaciones y páginas web de principio a fin: backend, base de datos y despliegue en producción
- 🐧 Apasionado del mundo Linux, la optimización y decoración de sistemas (uso frecuente de Arch Linux/CachyOS con entorno personalizado)
- 🎮 Amante de los videojuegos, el gimnasio y la vida sana

## 🛠️ Stack

**Lenguajes**

[![Languages](https://skillicons.dev/icons?i=java,bash,js,html,css)](https://skillicons.dev)

**Herramientas y tecnologías**

[![Tools](https://skillicons.dev/icons?i=spring,mongodb,mysql,aws,linux)](https://skillicons.dev)

## 📌 Proyectos destacados

### 🎬 PlotTwist
Aplicación web de catálogo de películas y series.
- Backend completo en Spring Boot con base de datos MongoDB Atlas
- Desplegada en producción sobre AWS EC2
- Frontend en HTML/CSS/JS puro, con foco en rendimiento

## 📊 Estadisticas de GitHub
<img src="https://github-readme-stats-nu-sable-12.vercel.app/api?username=MiguelSg77&show_icons=true&theme=transparent&hide_border=true&title_color=D8B4FE&icon_color=D8B4FE&text_color=B794F6&count_private=true" width="49%" /> <img src="https://github-readme-stats-nu-sable-12.vercel.app/api/top-langs/?username=MiguelSg77&layout=compact&theme=transparent&hide_border=true&title_color=D8B4FE&text_color=B794F6&count_private=true" width="49%" />
